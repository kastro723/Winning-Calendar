...
public class CalendarView extends View{
    ...
    static final int DRAG_THRESHOLD = 30; // Set your own value. This is just an example.
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(disX) > Math.abs(disY)){
                    if (Math.abs(disX) > DRAG_THRESHOLD) { // Check if drag distance is greater than threshold
            ...
            case MotionEvent.ACTION_UP:
                if (mScrolloffset.x == 0) {
                    if (Math.abs(mPrevX - x) <= DRAG_THRESHOLD && Math.abs(mPrevY - y) <= DRAG_THRESHOLD) {
                        // If the movement was not beyond the threshold, then consider this a tap
    ...
    private void drawWeekText(Canvas canvas) {
        ...
        canvas.drawText(mWeekStr.get(6), tempX,  mWeekHeight * 9 / 10, mPaint);
